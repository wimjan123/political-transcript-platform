...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <button
        type="button"
        className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200"
        ...
        <span className="flex items-center">
          ...
          <span className="block truncate">
            ...
            {selectedLang.code !== 'auto' && (
              <span className="text-gray-500 ml-1 dark:text-gray-400">({selectedLang.name})</span>
            )}
          </span>
        </span>
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm dark:bg-gray-800 dark:ring-gray-700">
            ...
            <button
              ...
              className={`${
                selectedLanguage === language.code
                  ? 'text-blue-900 bg-blue-100 dark:text-blue-300 dark:bg-blue-900/30'
            ...
            >
              <div className="flex items-center">
                <span className={`${selectedLanguage === language.code ? 'font-semibold' : 'font-normal'} block truncate`}>
                  {language.nativeName}
                  {language.code !== 'auto' && (
                    <span className="text-gray-500 ml-1 dark:text-gray-400">({language.name})</span>
                  )}
                </span>
              </div>
